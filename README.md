...
`retailUsdCents`，同步修改后后台、模拟订单和报价规则会保持一致。
...
新素材最早从第二天参与示例反馈匹配，到期由每日计划任务清理。后台确认授权的 TikTok/
小红书外链可以直接提取、去重并写入 R2。关键词自动采集使用 Jina Search + Reader 的公开
API，并需要在每个正式 Worker 中单独配置 Secret：

```bash
npx wrangler secret put JINA_API_KEY
```

定时任务默认每 3 天轮换一个关键词，每轮最多写入 3 张图片。自动发现的图片进入 R2 后
保持“待处理”，不会被公开接口返回，也不会自动匹配到客户反馈；管理员必须在素材后台查看
来源并确认商业展示授权后，才能把状态改为“可用”。TikSave 和 KuKuTool 仍只作为手动解析
辅助入口，系统不会绕过其验证码或调用未公开的私有下载接口。
