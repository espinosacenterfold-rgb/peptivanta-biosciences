...
function orderAllowed(user,o,customerMap){if(roleDef(user).scope==='all')return true;const c=o.customerId?customerMap.get(o.customerId):null;if(c)return canAccessCustomer(user,c);if(roleDef(user).scope==='owner')return o.owner===user.display_name;return false;}
function sanitizeIncoming(candidate){const out=safeClone(candidate);out.customers=Array.isArray(out.customers)?out.customers:[];out.orders=Array.isArray(out.orders)?out.orders:[];out.teams=Array.isArray(out.teams)?out.teams:[];return out;}
function normalizeNewCustomerOwnership(user,c){
  const x={...c},def=roleDef(user);x.createdByUserId=x.createdByUserId||Number(user.id);x.createdBy=x.createdBy||user.display_name;
  if(def.scope==='owner'){x.owner=user.display_name;x.ownerUserId=Number(user.id);x.team=user.team;return x;}
...
function mergeState(current,candidate,user){
  ...
  if(def.scope==='all'){
    const cur=byId(current.customers),customers=(candidate.customers||[]).map(c=>cur.has(c.id)?c:normalizeNewCustomerOwnership(user,c));
    const next={...current,...candidate,customers};next.accounts=current.accounts||[];next.permissions=current.permissions||[];next.whatsapp=current.whatsapp||[];return next;
  }
  ...
  for(const o of current.orders||[]){if(!orderAllowed(user,o,globalCustomerMap)){mergedOrders.push(o);continue;}const n=incomingOrders.get(o.id);if(!n){mergedOrders.push(o);continue;}const x={...o,...n};if(!hasPermission(user,'成本利润'))x.cost=o.cost;if(def.scope==='owner')x.owner=user.display_name;mergedOrders.push(x);}
...
function normalizeCustomerOwnerIds(state,users){
  const byName=new Map(users.map(u=>[u.display_name,u]));
  for(const c of(state.customers||[])){
    const u=byName.get(c.owner);if(!u)continue;c.ownerUserId=Number(u.id);
    if(u.team&&u.team!=='—'&&['普通销售','二级管理员 / 组长'].includes(u.permission_group))c.team=u.team;
  }
}
async function hydrateDbOwnedState(db,state,users){normalizeCustomerOwnerIds(state,users);state.accounts=usersToAccounts(users);state.permissions=Object.entries(ROLE_DEFS).map(([name,v],i)=>({id:`P-${i+1}`,name,scope:v.scope,permissions:v.permissions}));state.whatsapp=await listWhatsapps(db);return state;}
async function conflict(context,user){const fresh=await ensureState(context.env.DB),users=await listUsers(context.env.DB),state=await hydrateDbOwnedState(context.env.DB,parseState(fresh),users),scoped=scopeStateForUser(state,user,users);return json({ok:false,error:'revision_conflict',revision:Number(fresh.revision||1),state:scoped},409);}

export async function onRequestGet(context){try{const auth=await requireUser(context);if(auth.response)return auth.response;const row=await ensureState(context.env.DB),users=await listUsers(context.env.DB),state=await hydrateDbOwnedState(context.env.DB,parseState(row),users);return json({ok:true,state:scopeStateForUser(state,auth.user,users),revision:Number(row.revision||1),updatedAt:row.updated_at,scope:roleDef(auth.user).scope});}catch(e){return json({ok:false,error:'state_get_failed',message:e?.message||String(e)},500);}}
